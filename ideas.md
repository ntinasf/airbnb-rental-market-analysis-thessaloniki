...
i have created the report_draft.md file with all the markdown cells from all 4 analysis notebooks. now you will have to read it and create the final report.md file as per my instructions above. remember to include statistical validity statements (like effect sizes, variance explained, sample sizes) followed by the right visual to communicate it. use placeholders for the visuals. make sure to have smooth transitions between the different analysis topics and include clever headings for each section. finally create a closing that paints a bigger picture enhanced by our findings. do not forget to create a separate executive_summary.md file that fits your likining. do not use h3, h5 etc namings, those were just for our convenience. Remember that the reoport will accompany the power bi dashboard in my personal portfolio website so make sure it is worthy of publishing. Most of the text is written using my personal writting style. But i really like scott galloway's data storytelling style, so if you find it appropriate try to incorporate some touches of his style. Try to avoid big and dramatic statements but rather focus on clear, concise, and insightful storytelling backed by data. Remeber that almost every visual is followed by statistical tests. So include placeholders for findings that need to be filled by me later. 
